...
class BlurSuite extends FunSuite {
  ...
  test("HorizontalBoxBlur.parBlur with radius 1 should correctly blur all a 3x3 image") {
    val w = 3
    val h = 3
    val src = new Img(w, h)
    val dstB = new Img(w, h)
    val dstPb = new Img(w, h)
    src(0, 0) = 0;
    src(1, 0) = 1;
    src(2, 0) = 2
    src(0, 1) = 3;
    ...
    HorizontalBoxBlur.blur(src, dstB, 0, 3, 1)
    HorizontalBoxBlur.parBlur(src, dstPb, 3, 1)

    def check(x: Int, y: Int) =
      assert(dstB(x, y) == dstPb(x, y), s"(destination($x, $y) is wrong")
