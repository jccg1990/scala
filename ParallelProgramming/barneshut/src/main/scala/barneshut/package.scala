...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        val nwCy = centerY / 2
        val nSize = size / 2
        val newQuad = Fork(Empty(nwCx, nwCy, nSize), Empty(nwCx + nSize, nwCy, nSize), Empty(nwCx, nwCy + nSize, nSize), Empty(nwCx + nSize, nwCy + nSize, nSize))

        (bodies :+ b).foldLeft(newQuad)((q, b) => q.insert(b))
      } else {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val xP = {
        val i = math.max(((b.x - boundaries.minX) / sectorSize).toInt, 0)
        math.min(i, sectorPrecision - 1)
      }

      val yP = {
        val i = math.max(((b.y - boundaries.minY) / sectorSize).toInt, 0)
        math.min(i, sectorPrecision - 1)
      }
