...
import FloatOps._
  ...
  test("update boundaries should have min value"){
    val boundaries = new Boundaries()
    boundaries.minX = 5
    boundaries.minY = 5
    boundaries.maxX = 10
    ...
    val b = new Body(123f, 2f, 3f, 0f, 0f)
    val sim = new Simulator(new ExecutionContextTaskSupport, new TimeStatistics);
    sim.updateBoundaries(boundaries, b)

    assert(boundaries.minX == b.x)
    assert(boundaries.minY == b.y)
  }

  test("update boundaries should have max value"){
    val boundaries = new Boundaries()
    boundaries.minX = 5
    boundaries.minY = 5
    boundaries.maxX = 10
    ...
    val b = new Body(123f, 20f, 30f, 0f, 0f)
    val sim = new Simulator(new ExecutionContextTaskSupport, new TimeStatistics)
    sim.updateBoundaries(boundaries, b)

    assert(boundaries.maxX == b.x)
    assert(boundaries.maxY == b.y)
  }

  test("merge boundaries should have min and max value"){
    ...
    val sim = new Simulator(new ExecutionContextTaskSupport, new TimeStatistics)
    val newB = sim.mergeBoundaries(boundaries1, boundaries2)
