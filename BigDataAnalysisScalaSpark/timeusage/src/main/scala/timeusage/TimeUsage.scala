...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val fields = StructField(columnNames.head, StringType, nullable = false) :: columnNames.tail.map(fieldName => StructField(fieldName, DoubleType, nullable = false))
    StructType(fields)
  }
  ...
  def row(line: List[String]): Row = {
    Row.fromSeq(line.head :: line.tail.map(s => s.toDouble))
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val wA = List("t05", "t1805")
    val oA = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    columnNames.foldLeft((List[Column](), List[Column](), List[Column]()))((c, cN) =>
      if (pN.exists(prefix => cN.startsWith(prefix))) {
        (col(cN) :: c._1, c._2, c._3)
      } else if (wA.exists(prefix => cN.startsWith(prefix))) {
        (c._1, col(cN) :: c._2, c._3)
      } else if (oA.exists(prefix => cN.startsWith(prefix))) {
        (c._1, c._2, col(cN) :: c._3)
      } else {
        c
      }
    )
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when($"telfs".geq(1) && $"telfs".lt(3), "working").otherwise("not working").as("working")
    val sexProjection: Column = when($"tesex" === 1, "male").otherwise("female").as("sex")
    val ageProjection: Column = when($"teage".geq(15) && $"teage".leq(22), "young").when($"teage".geq(23) && $"teage".leq(55), "active").otherwise("elder").as("age")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    summed.groupByKey(r => (r.working, r.sex, r.age))
      ...
      .map {
        case ((w, s, a), p, wo, o) => TimeUsageRow(w, s, a, p, wo, o)
