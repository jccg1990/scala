...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    var leftE = left.isEmpty
    var rightE = right.isEmpty


    if(!leftE && left.mostRetweeted.retweets > elem.retweets){
      if(!rightE && right.mostRetweeted.retweets > elem.retweets){
        right.mostRetweeted
      } else{
        elem
      }
    } else if(!rightE && right.mostRetweeted.retweets > elem.retweets){
      right.mostRetweeted
    } else{
      elem
    }
