...
class FunSetSuite extends FunSuite {
  ...
  test("intersect contain all elements that are on all sets") {
    new TestSets {
  ...
  test("diff returns all elements that are not present in other set") {
    new TestSets {
      val s = diff(union(s2, s3), union(s1, s3))
  ...
  test("filter retuns subset of `s` for which `p` holds") {
    new TestSets {
  ...
  test("map Returns a set transformed by applying `f` to each element of `s`") {
    new TestSets {
      val s = union(s1, s2)
      val w = map(s, x => x+1)
      assert(!contains(w, 1), "Map 1")
      assert(contains(w, 2), "Map 2")
      assert(contains(w, 3), "Map 3")
    }
  }
